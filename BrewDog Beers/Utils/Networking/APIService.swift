...
class APIService: NSObject {
    ...
    func getDataWith(parameters: [Parameter:String]? = nil, completion: @escaping (Result<[[String: AnyObject]]>) -> Void){
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            guard error == nil else { return completion(.Error(error?.localizedDescription ?? "Invalid URL"))
            }
            guard let data = data else { return completion(.Error(error?.localizedDescription ?? "No data to show"))
            }
            do {
